...
# %% Load the matrix A
if N == 64:
    A_file_name = args.dataset_dir + "/A_3d" + str(N) + ".bin"
elif N == 128:
    A_file_name = args.dataset_dir + "/A_3d" + str(N) + ".bin"
print("Start loading matrixA from " + A_file_name)
start_matrix = time.time()
A = hf.readA_sparse_from_bin(N**3, A_file_name, 'f')
print(f"Shape of the 3D Poisson matrix: {A.shape}, dtype: {A.dtype}")
...
W, diagonal, sub_diagonal = MR.lanczos_iteration_with_normalization_correction(rand_vec,
                                                                               num_ritz_vectors)  # this can be loaded directly from c++ output
...
def apply_low_pass_filter(vector, frequency_cutoff_ratio):
    """
    ...
    cutoff = frequency_cutoff_ratio * np.max(np.abs(frequencies))
...
def apply_high_pass_filter(vector, frequency_cutoff_ratio):
    """
...
def generate_and_save_batches(num_batches, batch_size, freq_type, data_dir, start_index):
    ...
    for batch_num in range(num_batches):
        ...
        # Optional: Normalize x again after filtering
        for i in range(batch_size):
            x_norm = np.linalg.norm(x_temp_normalized[i]) + epsilon
            x_temp_normalized[i] /= x_norm
        # Step 5: Compute b = A * x (using filtered x)
        b_rhs_temp = np.zeros_like(x_temp_normalized)
        for i in range(batch_size):
            b_rhs_temp[i] = A.dot(x_temp_normalized[i])
        # Step 6: Save only b to files
        for i in range(batch_size):
            b_filename = os.path.join(data_dir, f'b_{sample_index}.npy')
            np.save(b_filename, b_rhs_temp[i].astype(np.float32))
            sample_index += 1
        t1 = time.time()
