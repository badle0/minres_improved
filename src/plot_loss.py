...
training_loss_file = '/data/hsheng/virtualenvs/minres_improved/lstsquares_filtered_training_fixed/3D_N64/minres_improved/3D_N64_training_loss.npy'
validation_loss_file = '/data/hsheng/virtualenvs/minres_improved/lstsquares_filtered_training_fixed/3D_N64/minres_improved/3D_N64_validation_loss.npy'

# Load the loss data from the .npy files
try:
    training_loss = np.load(training_loss_file)
    print(f"Training loss data loaded: {training_loss[:100]}")  # Print first 100 values for verification
...
try:
    ...
    print(f"Validation loss data loaded: {validation_loss[:100]}")  # Print first 100 values for verification
except Exception as e:
...
if validation_loss is not None and len(validation_loss) > 0:
    ...
    print(f"The best epoch is {best_epoch + 1} with a validation loss of {best_val_loss:.6f}")
else:
